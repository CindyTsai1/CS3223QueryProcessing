...
public class PlanCost {
	...
	/**
	 * If buffers are not enough for a selected join then this plan is not feasible
	 * and return a cost of infinity
	 **/

	boolean isFeasible;

	/**
	 * Hashtable stores mapping from Attribute name to number of distinct values of
	 * that attribute
	 **/
	...
	protected int getStatistics(Join node) {
		...
		/**
		 * get size of the tuple in output & correspondigly calculate buffer capacity,
	 ...
	 ** And statistics about the attributes Selection is performed on the fly, so no
	 * cost involved
	 **/
	...
	protected int getStatistics(Select node) {
		...
		/**
		 * Modify the number of distinct values of each attribute Assuming the values
		 * are distributed uniformly along entire relation
		 **/
	 ...
	 ** This table contains number of tuples in the table number of distinct values
	 * of each attribute
	 **/
	...
	protected int getStatistics(Scan node) {
		...
		// System.out.println("Scan: tablename="+tablename+"pres cost="+numpages+"total
		// cost="+cost);
		return numtuples;
	}
